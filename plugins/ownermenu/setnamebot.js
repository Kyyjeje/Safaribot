...
module.exports = {
    ...
    run: async ({ naze, m }) => {
        try {
            ...
            if (!newName) {
                return naze.sendMessage(m.chat, { text: `*Format salah!* Gunakan perintah:\n\n\`${m.prefix}${m.command} <nama baru>\`\nContoh: \`${m.prefix}${m.command} BangsulBotz Elite\`` }, { quoted: m });
            }

            await naze.updateProfileName(newName);

            await naze.sendMessage(m.chat, {
                text: 
                      `Nama bot Anda telah diperbarui dengan sukses!\n` +
                      `📛 *Nama Baru:* \`${newName}\``,
                quoted: m
            });
