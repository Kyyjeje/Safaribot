    ...
    run: async ({ naze, m }) => {
        try {
            ...
            if (text === 'on') {
                ...
                await naze.sendMessage(m.chat, { text: '*Anti Call Aktif!*' }, { quoted: m });
            } else if (text === 'off') {
                db.set[botNumber].anticall = false;
                
                await naze.sendMessage(m.chat, { text: '*Anti Call Tidak Aktif!*' }, { quoted: m });
            } else {
                
                await naze.sendMessage(m.chat, { text: `*Format salah!* Gunakan perintah:\n\n\`${m.prefix}${m.command} on\`\n> Untuk mengaktifkan anti call\n\n\`${m.prefix}${m.command} off\`\n> Untuk menonaktifkan anti call` }, { quoted: m });
            }
        } catch (err) {
