    ...
    run: async ({ naze, m }) => {
        try {
            ...
            if (text === 'on') {
                ...
                await naze.sendMessage(m.chat, { text: '*Auto Bio Aktif!*' }, { quoted: m });
            } else if (text === 'off') {
                db.set[botNumber].autobio = false;
                
                await naze.sendMessage(m.chat, { text: '*Auto Bio Tidak Aktif!*' }, { quoted: m });
            } else {
                
                await naze.sendMessage(m.chat, { text: `*Format salah!* Gunakan perintah:\n\n\`${m.prefix}${m.command} on\`\n> Untuk mengaktifkan auto bio\n\n\`${m.prefix}${m.command} off\`\n> Untuk menonaktifkan auto bio` }, { quoted: m });
            }
        } catch (err) {
