...
const fallbackQuestions = [
    { soal: "Aku selalu ada di depanmu, tetapi kamu tidak bisa melihat aku. Siapakah aku?", jawaban: "Masa depan" },
    { soal: "Aku bisa terbang tanpa sayap, aku bisa menangis tanpa mata. Apakah aku?", jawaban: "Awan" },
    { soal: "Semute apa yang paling genit?", jawaban: "Semute colek" }
];
...
module.exports = {
    ...
    run: async ({ naze, m }) => {
        ...
        if (games[m.chat]) {
            return m.reply('⚠️ *Permainan tebak-tebakan sudah aktif di chat ini!* Tunggu hingga selesai atau jawab soal yang ada.');
        }

        let soal, jawaban;
        try {
            const response = await fetch('https://api.siputzx.my.id/api/games/tebaktebakan', {
                headers: {
            ...
            console.error("[ERROR] Gagal mengambil soal tebak-tebakan:", e.message);
        ...
        try {
            ...
            const timerId = setTimeout(async () => {
                games = loadGames();
                if (games[m.chat] && games[m.chat].gameName === 'tebaktebakan') {
            ...
            console.error("[ERROR] Gagal memulai permainan tebak-tebakan:", e);
