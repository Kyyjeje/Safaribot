...
module.exports = {
    ...
    alias: ["cancel_giveaway", ".cancel_giveaway"],
    run: async ({ naze, m }) => {
        ...
        // Cek apakah user terdaftar di giveaway
        const userIndex = activeGiveaway.participants.findIndex(user => user.id === m.sender);
        if (userIndex === -1) {
            return naze.sendMessage(m.chat, { text: "❌ Kamu belum ikut giveaway!" }, { quoted: m });
        }

        // Hapus user dari daftar peserta
        activeGiveaway.participants.splice(userIndex, 1);
        saveGiveaways(giveaways);

        return naze.sendMessage(m.chat, { text: `✅ @${m.sender.split('@')[0]} berhasil membatalkan keikutsertaan giveaway!`, mentions: [m.sender] }, { quoted: m });
    }
};