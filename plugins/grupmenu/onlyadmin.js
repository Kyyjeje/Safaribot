...
module.exports = {
  ...
  run: async ({ naze, m }) => {
    try {
      ...
      if (body === 'onlyadmin_on') {
        action = 'on';
        isButtonCommand = true;
      } else if (body === 'onlyadmin_off') {
        action = 'off';
        isButtonCommand = true;
      } else {
      ...
      if (action === 'on') {
        if (global.db.groups[m.chat].onlyadmin) {
          
          await m.reply('Fitur onlyadmin sudah aktif sebelumnya.');
          return;
        }
        global.db.groups[m.chat].onlyadmin = true;
        
        await m.reply('Fitur onlyadmin berhasil diaktifkan.');
      } else if (action === 'off') {
        if (!global.db.groups[m.chat].onlyadmin) {
          
          await m.reply('Fitur onlyadmin sudah nonaktif sebelumnya.');
          return;
        }
        global.db.groups[m.chat].onlyadmin = false;
        
        await m.reply('Fitur onlyadmin berhasil dinonaktifkan.');
      } else {
      ...
      await m.reply('Gagal mengatur fitur onlyadmin.');
