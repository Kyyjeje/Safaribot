    ...
    try {
      ...
      if (action === 'on') {
        if (global.db.groups[m.chat].setinfo) {
          ...
          await m.reply('Fitur setinfo sudah aktif sebelumnya.');
          return;
        }
        global.db.groups[m.chat].setinfo = true;
        
        await m.reply('Fitur setinfo berhasil diaktifkan.');
      } else if (action === 'off') {
        if (!global.db.groups[m.chat].setinfo) {
          
          await m.reply('Fitur setinfo sudah nonaktif sebelumnya.');
          return;
        }
        global.db.groups[m.chat].setinfo = false;
        
        await m.reply('Fitur setinfo berhasil dinonaktifkan.');
      } else {
