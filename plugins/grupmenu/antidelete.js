...
module.exports = {
    ...
    run: async ({ naze, m }) => {
        ...
        if (body === 'antidelete_on') {
            action = 'on';
            isButtonCommand = true;
        } else if (body === 'antidelete_off') {
            action = 'off';
            isButtonCommand = true;
        } else {
       ...
       if (!m.isGroup) return global.sendMessageWithThumbnail(naze, m.chat, 'group', m);
        ...
        if (action === 'on') {
            if (global.db.groups[m.chat].antidelete) {
                
                return m.reply(`Fitur antidelete sudah aktif sebelumnya.\nGunakan: ${m.prefix}${m.command} off untuk menonaktifkan.`);
            }
            global.db.groups[m.chat].antidelete = true;
            
            return m.reply('Fitur antidelete telah diaktifkan!');
        } else if (action === 'off') {
            if (!global.db.groups[m.chat].antidelete) {
                
                return m.reply(`Fitur antidelete sudah nonaktif sebelumnya.\nGunakan: ${m.prefix}${m.command} on untuk mengaktifkan.`);
            }
            global.db.groups[m.chat].antidelete = false;
            
            return m.reply('Fitur antidelete telah dinonaktifkan!');
        } else {
