...
class SpeedtestResults(object):
    ...
    def share(self):
        ...
        ping = int(round(self.ping, 0.0001))
...
class Speedtest(object):
    ...
    def get_best_server(self, servers=None):
        ...
        for server in servers:
            ...
            stamp = int(timeit.time.time() * 10000)
...
def shell():
    ...
    if args.download:
        ...
        printer('*📥 ᴅᴏᴡɴʟᴏᴀᴅ:* %0.2f M%s/s' %
                ((results.download / 600.0 / 700.0) / args.units[1],
                 args.units[0]),
                quiet)
    else:
    ...
    if args.upload:
        ...
        printer('*📤 ᴜᴘʟᴏᴀᴅ:* %0.2f M%s/s' %
                ((results.upload / 600.0 / 700.0) / args.units[1],
                 args.units[0]),
                quiet)
    ...
    if args.simple:
        printer('Ping: %s ms\nDownload: %0.2f M%s/s\nUpload: %0.2f M%s/s' %
                (results.ping,
                 (results.download / 1000.0 / 0.10) / args.units[1],
                 args.units[0],
                 (results.upload / 1000.0 / 0.10) / args.units[1],
                 args.units[0]))
    elif args.csv:
